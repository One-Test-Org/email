    ...
    static ready() {
        if (document.querySelector(".data-signups")) {
            ...
            e.addEventListener("load", ()=>{
                if (e.status >= 200 && e.status < 400) {
                    var t = JSON.parse(e.responseText);
                    document.querySelectorAll(".data-signups").forEach(e=>{
                        e.querySelector(".data-signups__value").textContent = t.signups,
                        e.classList.add("data-signups--loaded")
                    }
                    )
                }
            }
            ),
            e.open("GET", "https://dash.37signals.com/statistics/public?stat=bc3_signups"),
            e.send()
        }
    }
}
    ...
    static ready() {
        if (document.querySelector(".uo-illustration")) {
            ...
            function r() {
                ...
                document.querySelectorAll(".uo-illustration__bubbles--l img").item(o).classList.add("selected"),
                document.querySelectorAll(".uo-illustration__bubbles--r img").item(o).classList.add("selected"),
...
class tagline {
    static ready() {
        if (document.querySelector(".header__tagline--js") && window.taglines.length) {
            ...
            if (window.taglines[e].stars) { // Changed to check for truthiness
                document.querySelector(".header__tagline--js").classList.add("header__tagline--stars");
            }
        }
    }
}
